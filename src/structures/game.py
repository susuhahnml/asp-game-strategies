...
class Game:
    ...
    def __init__(self, game_def):
        ...
        self.actionstr_to_idx = {symbol_str(a):i for
                                 i,a in enumerate(all_actions)}
    ...
    def current_observation(self):
        """
        The list defining the current obstervation with the size of all possible
        fluents. Contains 1 in the position of fluents that are true in
        the current state
        """
    ...
    def sample_random_legal_action(self):
        ...
        legal_action_str = symbol_str(self.current_state.legal_actions
                                      [r_l_idx].action)
    ...
    def step(self, player_str, action_idx):
        ...
        Accepts an action index and returns a tuple (observation, reward,
        done, info).
        Args:
        ...
        Returns
            ...
            current environment. Hot-One List
        ...
        if not legal_action:
            ...
            player_reward = -100
            print(paint_bool("••••••• EPISODE FINISHED Reward:{} •••••••".
                             format(player_reward),player_reward>0))
